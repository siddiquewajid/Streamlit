...
# Function to get suggested questions based on user profile
def get_suggested_questions(user_profile, all_questions, num_questions_to_suggest=10):
    eligible_questions_temp = []

    # 1. Add questions based on known programming languages
    for lang in user_profile.get("known_languages", []):
        eligible_questions_temp.extend(
            [q for q in all_questions if q.get("language_required") == lang]
        )

    # 2. Add questions based on course and branch
    course = user_profile.get("course")
    branch = user_profile.get("branch")

    if course == "B.Tech" or course == "MCA":
        if branch in ["CSE", "IT", "MCA"]:
            eligible_questions_temp.extend(
                questions_by_field.get("Network Security", []) +
                questions_by_field.get("Cloud Security", []) +
                questions_by_field.get("Application Security", []) +
                questions_by_field.get("Reverse Engineering", []) +
                questions_by_field.get("Penetration Testing", []) +
                questions_by_field.get("Cryptography", []) +
                questions_by_field.get("Malware Analysis", [])
            )
        elif branch in ["ECE", "EEE", "Other"]:
            eligible_questions_temp.extend(
                questions_by_field.get("IoT Security", []) +
                questions_by_field.get("Network Security", []) +
                # Assuming Embedded Security is related to ECE/EEE, if you have questions for it
                # questions_by_field.get("Embedded Security", []) +
                questions_by_field.get("Malware Analysis", [])
            )

    # 3. Remove duplicate questions (based on ID)
    seen_ids = set()
    unique_eligible_questions = []
    for q in eligible_questions_temp:
        if q["id"] not in seen_ids:
            unique_eligible_questions.append(q)
            seen_ids.add(q["id"])

    # 4. Fill remaining slots with random questions
    if len(unique_eligible_questions) < num_questions_to_suggest:
        remaining_questions = [q for q in all_questions if q["id"] not in seen_ids]
        random.shuffle(remaining_questions)
        needed = num_questions_to_suggest - len(unique_eligible_questions)
        unique_eligible_questions.extend(remaining_questions[:needed])

    random.shuffle(unique_eligible_questions)
    return unique_eligible_questions[:num_questions_to_suggest]
...
if st.session_state.quiz_started and not st.session_state.quiz_finished:
    ...
    if not st.session_state.suggested_questions:
        st.warning("Could not suggest any questions based on your profile. Here are some general questions instead.")
