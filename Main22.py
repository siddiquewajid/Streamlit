...
# Create a dictionary to group questions by field for efficient lookup
questions_by_field = {}
...
def get_suggested_questions(user_profile, all_questions, num_questions_per_field=10):
    suggested_questions_list = []
    seen_ids = set()

    # Define the fields you want to draw from
    # These are the keys in your 'questions_by_field' dictionary
    target_fields = [
        "Malware Analysis",
        "Penetration Testing",
        "Digital Forensics",
    ...
    for field in target_fields:
        if field in questions_by_field:
            ...
            # Filter out questions already added (though less critical with this approach if IDs are unique)
            available_field_questions = [q for q in field_questions if q["id"] not in seen_ids]

            # Randomly select 'num_questions_per_field' from the available questions
            if len(available_field_questions) >= num_questions_per_field:
                selected = random.sample(available_field_questions, num_questions_per_field)
            else:
                # If there are fewer than 'num_questions_per_field', take all of them
                selected = available_field_questions
            
            suggested_questions_list.extend(selected)
            for q in selected:
                seen_ids.add(q["id"])
        else:
            st.warning(f"Warning: No questions found for the field '{field}'.")

    # Shuffle the final list to randomize the order of questions from different fields
    random.shuffle(suggested_questions_list)
    return suggested_questions_list
...
courses = ["B.Tech", "MCA"]
user_course = st.sidebar.selectbox("Which course are you pursuing?", courses)

user_branch = "N/A"
if user_course == "B.Tech":
    branches = ["CSE", "IT", "ECE", "EEE", "Other"]
    user_branch = st.sidebar.selectbox("Which branch are you in (for B.Tech)?", branches)
...
user_known_languages_selected = st.sidebar.multiselect(
    ...
    all_available_languages + ["None of the above / Other"]
)
user_known_languages = [lang for lang in user_known_languages_selected if lang != "None of the above / Other"]
...
if st.sidebar.button("Save Profile and Start Quiz"):
    ...
    # Call with num_questions_per_field=10 to get 10 questions from each field
    st.session_state.suggested_questions = get_suggested_questions(
        st.session_state.user_profile, all_questions, num_questions_per_field=10
    )
    st.session_state.user_answers = {}
...
if st.session_state.quiz_started and not st.session_state.quiz_finished:
    ...
    if not st.session_state.suggested_questions:
        st.warning("No questions could be suggested based on the current configuration. Please check question data.")
        # Fallback to general random questions if no specific suggestions found
        st.session_state.suggested_questions = random.sample(all_questions, min(10, len(all_questions)))
    ...
    with st.form(key='quiz_form'):
        ...
        if submit_button:
            # Check if all suggested questions have been answered
    ...
    if field_scores:
        ...
        for field, score in field_scores.items():
            ...
            if max_for_field > 0: # Avoid division by zero
                ...
                st.write(f" - {field}: {score} correct (No questions from this field were included in the quiz)")
